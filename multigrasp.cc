...
Results find_answer(int capacity, vector<Customer> &customers, vector<vector<float>> &matrix) {
    ...
    while (visited_customers < customers.size() - 1) {
        ...
        for (int i = 0; i < customers.size(); i++) {    
            ...
            minimum_time = customers[i].ready_time - matrix[act_location][i] + act_time;
            while(!inserted) {
                // all times checked - push back actual
                ...
                // check time for actual solution
        ...
        if (visited_someone) {
            ...
            if (act_time < customers[next_location].ready_time) {
                act_time = customers[next_location].ready_time;
            } 
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < repetitions; i++) {
        ...
        if (results.trucks < best_trucks || (results.trucks == best_trucks && results.distance < best_distance)) {
            best_distance = results.distance;
            best_trucks = results.trucks;
